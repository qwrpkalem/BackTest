...
# --- v68: 피라미딩(불타기) ---
# 보유 중인 종목이 다시 신고가를 뚫으면 추가 매수한다. 진입 조건을 풀지 않고
# 자금 가동률을 올리는 유일한 방법 (평균 3.16종목 / 1년의 33.5% 무포지션).
PYRAMID = False              # v68 검증 -> 기각 (신호·수익 기준 모두 열세, 평균보유 오히려 감소)
PYRAMID_MAX = 2              # 포지션당 추가 매수 최대 횟수
PYRAMID_RATIO = 0.5          # 추가 매수 크기 (기존 투입금 대비)
PYRAMID_MIN_GAIN = 0.0       # ("signal" 모드) 현재 수익률이 이 이상일 때만 추가
PYRAMID_ON = "gain"          # "signal" = 재신호 시 / "gain" = 수익 계단마다
PYRAMID_STEP = 0.08          # ("gain" 모드) 이만큼 오를 때마다 한 번씩 추가
